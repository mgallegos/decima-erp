...
var u,s,y,w;var t,v;this.scope="multiselect"+(k++);this.optionGroupIndex=1;this._setLocale(this.options.locale);this.element.addClass("uix-multiselect-original");this._elementWrapper=d("<div></div>").addClass("uix-multiselect ui-widget").css({width:this.element.outerWidth(),height:this.element.outerHeight()}).append(d("<div></div>").addClass("multiselect-selected-list").append(d("<div></div>").addClass("ui-widget-header").append(v=d("<button><i class='fa fa-fast-forward'></button>",{type:"button"}).addClass("uix-control-right btn btn-default btn-xs").attr("data-localekey","deselectAll").attr("title",this._t("deselectAll")).button({icons:{primary:"ui-icon-arrowthickstop-1-e"},text:false}).click(function(z){z.preventDefault();
z.stopPropagation();x.optionCache.setSelectedAll(false);return false})).append(u=d("<div></div>").addClass("header-text multiselect-header"))).append(s=d("<div></div>").addClass("uix-list-container ui-widget-content")))["right,top".indexOf(this.options.availableListPosition)>=0?"prepend":"append"](d("<div></div>").addClass("multiselect-available-list").append(d("<div></div>").addClass("ui-widget-header").append(t=d("<button><i class='fa fa-fast-backward'></button>",{type:"button"}).addClass("uix-control-right btn btn-default btn-xs").attr("data-localekey","selectAll").attr("title",this._t("selectAll")).button({icons:{primary:"ui-icon-arrowthickstop-1-w"},text:false}).click(function(z){z.preventDefault();
z.stopPropagation();x.optionCache.setSelectedAll(true);return false})).append(y=d("<div></div>").addClass("header-text multiselect-header"))).append(w=d("<div></div>").addClass("uix-list-container ui-widget-content"))).insertAfter(this.element);this._buttons={selectAll:t,deselectAll:v};
