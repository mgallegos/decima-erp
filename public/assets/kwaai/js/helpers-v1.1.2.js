...
function empty(mixed_var)
{
  ...
  var undef, key, i, len;
  var emptyValues = [undef, null, false, 0, '', '0', '0.0', '0.00', '0.000', '0.0000', '0.00000', '0.000000'];
...
function decNumberToText(c, upperCase = true, currencyName = '')
{
	let cents = amount = 0;

	cents = Math.round((c - Math.floor(c)) * 100)
	amount = Math.floor(c)
	...
	if (!upperCase) 
	{
		return decNumberToTextAux(amount, 0) + cents + '/100 ' + currencyName
	} 
	else 
	{
		return (decNumberToTextAux(amount, 0) + cents + '/100 ' + currencyName).toUpperCase()
	}
}

// function decNumberToTextAux(number, thousand)
// {
// 	let exactNumbers = {
...
// 	if (number in exactNumbers && number != 100)
// 	{
// 		return exactNumbers[number]
// 	}
...
// 	if (number < 100) 
// 	{
// 		return dozensHundreds[number - (number % 10)] + exactNumbers[number % 10]
// 	} 
// 	else if (number == 100) 
// 	{
...
// 	else if (number < 1000) 
// 	{
// 		return exactNumbers[number - (number % 100)] + decNumberToTextAux((number % 100), 0)
// 	}
...
// 	if (((number - (number % 1000)) / 1000) == 1) 
// 	{
// 		return '{{Lang::get('numbers.un')}}' + threeFigures[thousand + 1].replace('{{Lang::get('numbers.es')}}', "") + (number%1000 != 0? decNumberToTextAux((number % 1000), 0) : '');
// 	} 
// 	else 
// 	{
// 		if ((((number  - (number %1000)) / 1000) % 1000) != 0 || thousand % 2 != 0) 
// 		{
// 			return decNumberToTextAux((number - (number % 1000)) / 1000, thousand + 1) + threeFigures[thousand + 1] + (number%1000 != 0? decNumberToTextAux((number % 1000), 0) : ''); 
// 		}
// 		else 
// 		{
// 			return decNumberToTextAux((number - (number % 1000)) / 1000, thousand + 1) + (number%1000 != 0? decNumberToTextAux((number % 1000), 0) : '');
// 		}
// 	}
// }
